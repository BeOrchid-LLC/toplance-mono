...
**Deployment note.** Neither change populates `fx_rates` on its own. Staging
needs `/api/cron/fx-rates` on a daily schedule (a Coolify scheduled task with
the `Authorization: Bearer $CRON_SECRET` header, the same shape as the digest
and VisaList jobs), and `OPEN_EXCHANGE_RATES_APP_ID` set in that environment.
`npm run fx:refresh` is the manual equivalent for a one-off fill. Without one
of the two, the figure the client asked for does not exist on any environment
— which is the actual state the demo was in.

`fx:refresh` runs the app's own `refreshFxRates()` rather than a copy of it,
which needs `scripts/alias-hook.mjs` (teaches plain `node` the `@/` alias that
`--experimental-strip-types` does not read from `tsconfig.json`) and
`--conditions=react-server` (resolves `server-only` to its empty build instead
of the module that throws outside a bundler). Both are reusable by any future
script that needs to reach into `src/`.
